//! Light palette implementation.

use super::style_id::StyleId;
use super::{Palette, PaletteStyle};
use crate::cell::{Color, Style};

const fn ansi(n: u8) -> Color {
    Color::Ansi(n)
}

pub struct LightPalette;

impl Palette for LightPalette {
    fn style(&self, id: StyleId) -> Style {
        match id {
            StyleId::Text => Style::default(),
            StyleId::Dim => PaletteStyle::fg(ansi(7)).to_style(),
            StyleId::Bright => PaletteStyle::fg(ansi(0)).to_style(),
            StyleId::Border => PaletteStyle::new().bold().to_style(),
            StyleId::Separator => PaletteStyle::fg(ansi(7)).to_style(),
            StyleId::TreeDir => PaletteStyle::fg(ansi(4)).to_style(),
            StyleId::CursorFocused => PaletteStyle::bg(ansi(12)).underline().to_style(),
            StyleId::CursorUnfocused => PaletteStyle::bg(ansi(7)).to_style(),
            StyleId::InputCursor => PaletteStyle::colors(ansi(15), ansi(0)).to_style(),
            StyleId::EditOverlay => PaletteStyle::colors(ansi(0), ansi(11)).to_style(),
            StyleId::EditSelection => PaletteStyle::bg(ansi(10)).to_style(),
            StyleId::SearchMatch => PaletteStyle::bg(ansi(11)).to_style(),
            StyleId::VisualSelection => PaletteStyle::colors(ansi(4), ansi(15)).to_style(),
            StyleId::Disabled => PaletteStyle::fg(ansi(7)).to_style(),
            StyleId::ChromeBar => PaletteStyle::colors(ansi(0), ansi(7)).to_style(),
            StyleId::TabFocused => PaletteStyle::colors(ansi(4), ansi(12)).bold().to_style(),
            StyleId::TabFocusedArrow => PaletteStyle::colors(ansi(4), ansi(12)).to_style(),
            StyleId::TabFocusedBadge => PaletteStyle::colors(ansi(15), ansi(4)).bold().to_style(),
            StyleId::TabActive => PaletteStyle::colors(ansi(0), ansi(7)).bold().to_style(),
            StyleId::TabActiveArrow => PaletteStyle::colors(ansi(0), ansi(7)).to_style(),
            StyleId::TabActiveBadge => PaletteStyle::colors(ansi(0), ansi(7)).to_style(),
            StyleId::TabInactive => PaletteStyle::colors(ansi(0), Color::Rgb(0xd0, 0xd0, 0xd0)).to_style(),
            StyleId::StatusBar => PaletteStyle::colors(ansi(0), ansi(7)).to_style(),
            StyleId::StatusBarModal => PaletteStyle::colors(ansi(15), ansi(4)).to_style(),
            StyleId::ScrollbarTrack => PaletteStyle::fg(ansi(7)).to_style(),
            StyleId::ScrollbarThumb => PaletteStyle::colors(ansi(15), ansi(0)).to_style(),
            StyleId::PopupBackground => PaletteStyle::colors(ansi(0), ansi(15)).to_style(),
            StyleId::PopupBorder => PaletteStyle::colors(ansi(4), ansi(15)).to_style(),
            StyleId::PopupSelected => PaletteStyle::colors(ansi(15), ansi(4)).underline().to_style(),
            StyleId::PopupTableHeader => PaletteStyle::colors(ansi(15), ansi(0)).bold().to_style(),
            StyleId::StateError => PaletteStyle::fg(ansi(9)).to_style(),
            StyleId::StateWarning => PaletteStyle::fg(ansi(11)).to_style(),
            StyleId::StateInfo => PaletteStyle::fg(ansi(14)).to_style(),
            StyleId::StateSuccess => PaletteStyle::fg(ansi(2)).to_style(),
            StyleId::StateHint => PaletteStyle::fg(ansi(8)).to_style(),
            StyleId::EditorGutter => PaletteStyle::fg(ansi(7)).to_style(),
        }
    }
}
